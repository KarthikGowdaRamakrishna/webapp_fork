...
export const deleteProfilePicService = async (userId) => {
  ...
  const deleteParams = {
    Bucket: process.env.S3_BUCKET,
    Key: `profile-pics/${userId}/${profilePic.file_name}`, // Adjusted for user-specific folder
  };

  await deleteFromS3(deleteParams); // Delete from S3 with the user-specific path
